...
int main(int argc, char **argv) {
  ...
  err = set_max_entries(acl->maps.rule_tbl, argv[2]);
  if (err) {
    fprintf(stderr, "Failed to set max entries\n");
    return 1;
  }

  err = acl_bpf__load(acl);
  if (err) {
    fprintf(stderr, "Failed to load BPF object\n");
    return 1;
  }
  err = fill_tbl(acl->maps.rule_tbl, argv[2]);
  if (err) {
    fprintf(stderr, "Failed to fill external map\n");
    return 1;
  }

  err = bpf_xdp_attach(if_index, bpf_program__fd(acl->progs.acl), 0, NULL);
  if (err) {
