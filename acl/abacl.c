...
int if_index;
struct abacl_bpf *abacl;

int mask = 1;

void sig_handler(int sig) {
  bpf_xdp_detach(if_index, 0, NULL);
  abacl_bpf__destroy(abacl);
  exit(0);
}
...
int main(int argc, char **argv) {
  ...
  err = set_max_entries(abacl->maps.rule_tbl, argv[2]);
  if (err) {
    fprintf(stderr, "Failed to set max entries\n");
    return 1;
  }

  err = abacl_bpf__load(abacl);
  if (err) {
    fprintf(stderr, "Failed to load BPF object\n");
    return 1;
  }
  err = fill_tbl(abacl->maps.rule_tbl, argv[2]);
  if (err) {
    fprintf(stderr, "Failed to fill external map\n");
    return 1;
  }

  err = bpf_xdp_attach(if_index, bpf_program__fd(abacl->progs.abacl), 0, NULL);
  if (err) {
