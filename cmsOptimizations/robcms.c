...
#include "robcms.bpf.skel.h"

int if_index;
struct robcms_bpf *robcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	robcms_bpf__destroy(robcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	robcms = robcms_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(robcms->progs.robcms), 0, NULL);
	if (err)
	{
