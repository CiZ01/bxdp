...
#include "rcms.bpf.skel.h"

int if_index;
struct rcms_bpf *rcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	rcms_bpf__destroy(rcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(rcms->progs.rcms), 0, NULL);
	if (err)
	{
