...
#include "rbcms.bpf.skel.h"

int if_index;
struct rbcms_bpf *rbcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	rbcms_bpf__destroy(rbcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	rbcms = rbcms_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(rbcms->progs.rbcms), 0, NULL);
	if (err)
	{
