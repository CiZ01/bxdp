...
#include "bcms.bpf.skel.h"

int if_index;
struct bcms_bpf *bcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	bcms_bpf__destroy(bcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(bcms->progs.bcms), 0, NULL);
	if (err)
	{
