...
#include "sobcms.bpf.skel.h"

int if_index;
struct sobcms_bpf *sobcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	sobcms_bpf__destroy(sobcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	sobcms = sobcms_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(sobcms->progs.sobcms), 0, NULL);
	if (err)
	{
