...
#include "bloadbalancer.bpf.skel.h"
#include "common.h"

int if_index;
struct bloadbalancer_bpf *bloadbalancer;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	bloadbalancer_bpf__destroy(bloadbalancer);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	bloadbalancer = bloadbalancer_bpf__open();//_and_load();
	...
	err = bloadbalancer_bpf__load(bloadbalancer);
	if (err)
	{
	...
	err = fill_map(bloadbalancer->maps.servers, argv[2]);


	err = bpf_xdp_attach(if_index, bpf_program__fd(bloadbalancer->progs.bloadbalancer), 0, NULL);
	if (err)
	{
