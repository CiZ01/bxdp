...
#include "loadbalancer.bpf.skel.h"
#include "common.h"

int if_index;
struct loadbalancer_bpf *loadbalancer;
#define MAX_DEST 512

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	loadbalancer_bpf__destroy(loadbalancer);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	loadbalancer = loadbalancer_bpf__open();//_and_load();
	...
	err = loadbalancer_bpf__load(loadbalancer);
	if (err)
	{
	...
	err = fill_map(loadbalancer->maps.servers, argv[2]);


	err = bpf_xdp_attach(if_index, bpf_program__fd(loadbalancer->progs.loadbalancer), 0, NULL);
	if (err)
	{
