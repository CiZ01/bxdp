...
#include "tconntrack.bpf.skel.h"
#include "conntrack_bpf_log.h"
#include "conntrack_common.h"


int if_index;
struct tconntrack_bpf *tconntrack;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	tconntrack_bpf__destroy(tconntrack);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	// tconntrack = tconntrack_bpf__open_and_load();
	// if (!tconntrack)
	// {
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(skel->progs.tconntrack), 0, NULL);
	if (err)
	{
