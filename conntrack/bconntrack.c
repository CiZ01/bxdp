...
#include "bconntrack.bpf.skel.h"
#include "conntrack_bpf_log.h"
#include "conntrack_common.h"


int if_index;
struct bconntrack_bpf *bconntrack;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	bconntrack_bpf__destroy(bconntrack);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	// bconntrack = bconntrack_bpf__open_and_load();
	// if (!bconntrack)
	// {
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(skel->progs.bconntrack), 0, NULL);
	if (err)
	{
