...
#include "counter.bpf.skel.h"

int if_index;
struct counter_bpf *counter;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	counter_bpf__destroy(counter);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	counter = counter_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(counter->progs.counter), 0, NULL);
	if (err)
	{
