...
#include "bpcounter.bpf.skel.h"

int if_index;
struct bpcounter_bpf *bpcounter;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	bpcounter_bpf__destroy(bpcounter);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	bpcounter = bpcounter_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(bpcounter->progs.bpcounter), 0, NULL);
	if (err)
	{
