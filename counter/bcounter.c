...
#include "bcounter.bpf.skel.h"

int if_index;
struct bcounter_bpf *bcounter;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	bcounter_bpf__destroy(bcounter);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	bcounter = bcounter_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(bcounter->progs.bcounter), 0, NULL);
	if (err)
	{
