...
int if_index;
struct bsampler_bpf *bsampler;


void sig_handler(int sig) {
  bpf_xdp_detach(if_index, 0, NULL);
  bsampler_bpf__destroy(bsampler);
  exit(0);
}
...
int main(int argc, char **argv) {
  ...
  bsampler = bsampler_bpf__open_and_load();
  ...
  err = bpf_xdp_attach(if_index, bpf_program__fd(bsampler->progs.bsampler), 0, NULL);
  if (err) {
