...
int if_index;
struct sampler_bpf *sampler;


void sig_handler(int sig) {
  bpf_xdp_detach(if_index, 0, NULL);
  sampler_bpf__destroy(sampler);
  exit(0);
}
...
int main(int argc, char **argv) {
  ...
  sampler = sampler_bpf__open_and_load();
  ...
  err = bpf_xdp_attach(if_index, bpf_program__fd(sampler->progs.sampler), 0, NULL);
  if (err) {
