...
int if_index;
struct obsampler_bpf *obsampler;


void sig_handler(int sig) {
  bpf_xdp_detach(if_index, 0, NULL);
  obsampler_bpf__destroy(obsampler);
  exit(0);
}
...
int main(int argc, char **argv) {
  ...
  obsampler = obsampler_bpf__open_and_load();
  ...
  err = bpf_xdp_attach(if_index, bpf_program__fd(obsampler->progs.obsampler), 0, NULL);
  if (err) {
