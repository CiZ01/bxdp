...
int if_index;
struct obnat_bpf *obnat;


void sig_handler(int sig) {
  bpf_xdp_detach(if_index, 0, NULL);
  obnat_bpf__destroy(obnat);
  exit(0);
}
...
int main(int argc, char **argv) {
  ...
  obnat = obnat_bpf__open_and_load();
  ...
  err = fill_tbl(obnat->maps.external_map, argv[2]);
  if (err) {
    fprintf(stderr, "Failed to fill external map\n");
    return 1;
  }

  err = bpf_xdp_attach(if_index, bpf_program__fd(obnat->progs.obnat), 0, NULL);
  if (err) {
