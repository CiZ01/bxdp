...
#include "obconntrack.bpf.skel.h"
#include "conntrack_bpf_log.h"
#include "conntrack_common.h"


int if_index;
struct obconntrack_bpf *obconntrack;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	obconntrack_bpf__destroy(obconntrack);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	// obconntrack = obconntrack_bpf__open_and_load();
	// if (!obconntrack)
	// {
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(skel->progs.obconntrack), 0, NULL);
	if (err)
	{
