...
#include "obcms.bpf.skel.h"

int if_index;
struct obcms_bpf *obcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	obcms_bpf__destroy(obcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	obcms = obcms_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(obcms->progs.obcms), 0, NULL);
	if (err)
	{
