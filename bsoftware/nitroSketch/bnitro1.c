...
#include "bnitro1.bpf.skel.h"

int if_index;
struct bnitro1_bpf *bnitro1;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	bnitro1_bpf__destroy(bnitro1);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	bnitro1 = bnitro1_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(bnitro1->progs.bnitro1), 0, NULL);
	if (err)
	{
