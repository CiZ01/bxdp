...
int tpobnat(struct xdp_md *ctx) {
    ...
    ip3 = *nat_ip3; 
    ip4 = *nat_ip4; 
    int ret1 = update_ipaddr(data, data_end, ip1);
    int ret2 = update_ipaddr(data + (lens[0] & 0x1FFF), data_end, ip2);
    int ret3 = update_ipaddr(data + (lens[0] +lens[1] & 0x1FFF), data_end, ip3);
    int ret4 = update_ipaddr(data + (lens[0] +lens[1] +lens[2] & 0x1FFF), data_end, ip4);
