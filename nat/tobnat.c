...
int if_index;
struct tobnat_bpf *tobnat;


void sig_handler(int sig) {
  bpf_xdp_detach(if_index, 0, NULL);
  tobnat_bpf__destroy(tobnat);
  exit(0);
}
...
int main(int argc, char **argv) {
  ...
  tobnat = tobnat_bpf__open_and_load();
  ...
  err = fill_tbl(tobnat->maps.external_map, argv[2]);
  if (err) {
    fprintf(stderr, "Failed to fill external map\n");
    return 1;
  }

  err = bpf_xdp_attach(if_index, bpf_program__fd(tobnat->progs.tobnat), 0, NULL);
  if (err) {
