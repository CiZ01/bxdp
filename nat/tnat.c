...
int if_index;
struct tnat_bpf *tnat;


void sig_handler(int sig) {
  bpf_xdp_detach(if_index, 0, NULL);
  tnat_bpf__destroy(tnat);
  exit(0);
}
...
int main(int argc, char **argv) {
  ...
  err = fill_tbl(tnat->maps.external_map, argv[2]);
  if (err) {
    fprintf(stderr, "Failed to fill external map\n");
    return 1;
  }

  err = bpf_xdp_attach(if_index, bpf_program__fd(tnat->progs.tnat), 0, NULL);
  if (err) {
