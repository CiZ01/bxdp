...
int if_index;
struct tpobnat_bpf *tpobnat;


void sig_handler(int sig) {
  bpf_xdp_detach(if_index, 0, NULL);
  tpobnat_bpf__destroy(tpobnat);
  exit(0);
}
...
int main(int argc, char **argv) {
  ...
  tpobnat = tpobnat_bpf__open_and_load();
  ...
  err = fill_tbl(tpobnat->maps.external_map, argv[2]);
  if (err) {
    fprintf(stderr, "Failed to fill external map\n");
    return 1;
  }

  err = bpf_xdp_attach(if_index, bpf_program__fd(tpobnat->progs.tpobnat), 0, NULL);
  if (err) {
