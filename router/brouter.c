...
#include "brouter.bpf.skel.h"

int if_index;
struct brouter_bpf *brouter;
...
void updatelpm() {
    ...
    for(__u8 i=8; i<=32;i++){
        ...
        while ((read = getline(&line, &len, fp)) != -1) {
            ...
            assert(bpf_map__update_elem(brouter->maps.lpm, &key,sizeof(struct ipv4_lpm_key), &i,sizeof(i), BPF_ANY)==0);
            count++;
            counttot++;
        }
        fclose(fp);
...
int main(int argc, char **argv)
{
	...
	brouter = brouter_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(brouter->progs.brouter), 0, NULL);
	if (err)
	{
