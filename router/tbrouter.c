...
#include "tbrouter.bpf.skel.h"

int if_index;
struct tbrouter_bpf *tbrouter;
...
void sig_handler(int sig)
{
	...
	tbrouter_bpf__destroy(tbrouter);
	exit(0);
}
...
void updatelpm() {
    ...
    for(__u8 i=8; i<=32;i++){
        ...
        while ((read = getline(&line, &len, fp)) != -1) {
            ...
            assert(bpf_map__update_elem(tbrouter->maps.lpm, &key,sizeof(struct ipv4_lpm_key), &i,sizeof(i), BPF_ANY)==0);
            count++;
            counttot++;
        }
        fclose(fp);
...
int main(int argc, char **argv)
{
	...
	tbrouter = tbrouter_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(tbrouter->progs.tbrouter), 0, NULL);
	if (err)
	{
