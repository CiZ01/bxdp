...
#include "obnitro1.bpf.skel.h"

int if_index;
struct obnitro1_bpf *obnitro1;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	obnitro1_bpf__destroy(obnitro1);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	obnitro1 = obnitro1_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(obnitro1->progs.obnitro1), 0, NULL);
	if (err)
	{
