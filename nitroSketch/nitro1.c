...
#include "nitro1.bpf.skel.h"

int if_index;
struct nitro1_bpf *nitro1;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	nitro1_bpf__destroy(nitro1);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	nitro1 = nitro1_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(nitro1->progs.nitro1), 0, NULL);
	if (err)
	{
