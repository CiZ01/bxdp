...
int if_index;
struct bnitro2_bpf *bnitro2;

void sig_handler(int sig) {
  bpf_xdp_detach(if_index, 0, NULL);
  bnitro2_bpf__destroy(bnitro2);
  exit(0);
}
...
int fill_geo(struct bpf_map *map) {
  ...
  for (int i = 0; i < 4096; i++) { // samplig rate
    gs.geo_sampling_array[i] = gsl_ran_geometric(r, 0.06);
...
//   }
  ...
  int key = 0;
  int ret =
      bpf_map__update_elem(map, &key, sizeof(key), &gs, sizeof(gs), BPF_ANY);
  if (ret) {
...
int main(int argc, char **argv) {
  ...
  bnitro2 = bnitro2_bpf__open_and_load();
  ...
  err = fill_geo(bnitro2->maps.geosampling);
  if (err) {
  ...
  err = bpf_xdp_attach(if_index, bpf_program__fd(bnitro2->progs.bnitro2), 0,
                       NULL);
  if (err) {
