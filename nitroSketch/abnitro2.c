...
int if_index;
struct abnitro2_bpf *abnitro2;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	abnitro2_bpf__destroy(abnitro2);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	abnitro2 = abnitro2_bpf__open_and_load();
	...
	err = fill_geo(abnitro2->maps.geosampling);
	if (err) {
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(abnitro2->progs.abnitro2), 0, NULL);
	if (err)
	{
