...
int if_index;
struct obnitro2_bpf *obnitro2;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	obnitro2_bpf__destroy(obnitro2);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	obnitro2 = obnitro2_bpf__open_and_load();
	...
	err = fill_geo(obnitro2->maps.geosampling);
	if (err) {
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(obnitro2->progs.obnitro2), 0, NULL);
	if (err)
	{
