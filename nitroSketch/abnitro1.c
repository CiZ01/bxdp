...
#include "abnitro1.bpf.skel.h"

int if_index;
struct abnitro1_bpf *abnitro1;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	abnitro1_bpf__destroy(abnitro1);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	abnitro1 = abnitro1_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(abnitro1->progs.abnitro1), 0, NULL);
	if (err)
	{
