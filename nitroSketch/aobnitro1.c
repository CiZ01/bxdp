...
#include "aobnitro1.bpf.skel.h"

int if_index;
struct aobnitro1_bpf *aobnitro1;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	aobnitro1_bpf__destroy(aobnitro1);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	aobnitro1 = aobnitro1_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(aobnitro1->progs.aobnitro1), 0, NULL);
	if (err)
	{
