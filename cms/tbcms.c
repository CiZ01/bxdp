...
#include "tbcms.bpf.skel.h"

int if_index;
struct tbcms_bpf *tbcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	tbcms_bpf__destroy(tbcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	tbcms = tbcms_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(tbcms->progs.tbcms), 0, NULL);
	if (err)
	{
