...
#include "abcms.bpf.skel.h"

int if_index;
struct abcms_bpf *abcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	abcms_bpf__destroy(abcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	abcms = abcms_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(abcms->progs.abcms), 0, NULL);
	if (err)
	{
