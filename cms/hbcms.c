...
#include "hbcms.bpf.skel.h"

int if_index;
struct hbcms_bpf *hbcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	hbcms_bpf__destroy(hbcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	hbcms = hbcms_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(hbcms->progs.hbcms), 0, NULL);
	if (err)
	{
