...
#include "aobcms.bpf.skel.h"

int if_index;
struct aobcms_bpf *aobcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	aobcms_bpf__destroy(aobcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	aobcms = aobcms_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(aobcms->progs.aobcms), 0, NULL);
	if (err)
	{
