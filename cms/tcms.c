...
#include "tcms.bpf.skel.h"

int if_index;
struct tcms_bpf *tcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	tcms_bpf__destroy(tcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(tcms->progs.tcms), 0, NULL);
	if (err)
	{
