...
#include "ibcms.bpf.skel.h"



int if_index;
struct ibcms_bpf *ibcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	ibcms_bpf__destroy(ibcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	ibcms = ibcms_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(ibcms->progs.ibcms), 0, NULL);
	if (err)
	{
