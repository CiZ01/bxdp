...
#include "ubcms.bpf.skel.h"

int if_index;
struct ubcms_bpf *ubcms;

void sig_handler(int sig)
{
	bpf_xdp_detach(if_index, 0, NULL);
	ubcms_bpf__destroy(ubcms);
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	ubcms = ubcms_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(ubcms->progs.ubcms), 0, NULL);
	if (err)
	{
